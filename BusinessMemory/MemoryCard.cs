...
namespace BusinessMemory
{
    public class MemoryCard<T>
    {
        ...
        public static MemoryCard<T>[][] CreateGrid(int numberOfUniqueCards)
        {
            ...
            SetIds(memoryCards);
            var result = GetRowsColsExtra(memoryCards);
        ...
        private static (int, int, int) GetRowsColsExtra(MemoryCard<T>[] memoryCards)
        {
            int numberOfCards = memoryCards.Length*2;
            ...
            else
            {
                ...
                int extra = numberOfCards - (rows * cols);
        ...
        private static MemoryCard<T>[][] GetMemoryCards(int rows, int cols, int extra, MemoryCard<T>[] memoryCards, int colsCoordinates)
        {
            ...
            int originalcards = rows + cols + extra / 2;

            Dictionary<MemoryCard<T>, int> pickedCardsAmount = new Dictionary<MemoryCard<T>, int>();
        ...
        private static MemoryCard<T> GetRandomMemoryCard(MemoryCard<T>[] initalCards, Dictionary<MemoryCard<T>, int> pickedCardsAmount)
        {
            //get random card from memorycards
            Random random = new Random();
            int rand = random.Next(initalCards.Length);
            if (!pickedCardsAmount.ContainsKey(initalCards[rand]))
            {
                pickedCardsAmount[initalCards[rand]] = 1;
                return initalCards[rand];
            }
            else
            {
                if (pickedCardsAmount[initalCards[rand]] < 2)
                {
                    pickedCardsAmount[initalCards[rand]]++;
                    return initalCards[rand];
                }
